...
# %%
SFT_FILES = ['./datasets/aplca1.parquet',
...
# %%
dataset = load_dataset(path='parquet', data_files=SFT_FILES, split='train', keep_in_memory=False)
...
def format_example(example):
    ...
    if example.get("input"):
        ...
        context=prompt_input.format_map(dict(instruction=example['instruction'],input=example['input']))
...
tokenized_datasets = tokenized_datasets.map(function=preprocess,num_proc=32, keep_in_memory=False).shuffle(23333)
print("2")
print(tokenized_datasets)
# %% [markdown]
# ## 2.2 定义data_collator
...
l#oss_log.to_csv(f"./logs/sft_train_log_{time.strftime('%Y%m%d-%H%M')}.csv")
